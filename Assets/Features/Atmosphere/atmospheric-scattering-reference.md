...
OpticalDepthLUT (256×256, RG16Half)
    ...
    │       └─→ SkyViewLut (512×256, ARGBFloat) ──→ 天空盒
    │       │       │
    │       │       └─→ SHConvolution ──→ unity_SHAr..SHC (全局 ambient probe)
    │       │
    │       └─→ AerialPerspectiveLUT (128×72×64, ARGBHalf) ──→ 场景物体
```
...
### 2.4 SH Convolution（天光球谐）

**用途**：把 Sky View LUT 卷积成 2nd-order 球谐系数（9 个 RGB 系数），覆盖 Unity 的 `unity_SHAr..unity_SHC` 全局 ambient probe，使场景中所有对象的间接光照实时响应大气散射变化。

**原理** — Monte Carlo 球面积分：

```
c_lm = ∫ L(ω) · Y_lm(ω) dω ≈ (4π/N) · Σ_i L(ω_i) · Y_lm(ω_i)
```

其中 L(ω) 是 SkyViewLut 在方向 ω 的辐射度，Y_lm 是实 SH 基函数，N = LUT 总像素数。

**实现** — 两级并行 tile reduction：

| Stage | Kernel | Dispatch | 工作 |
|-------|--------|----------|------|
| 1 | `CSTileSH` | `(32, 16, 1)` groups, `16×16` threads | 每个线程处理 1 个 LUT texel → 组内共享内存 tree reduce → 512 个部分 SH 结果写入 `_TileSH` |
| 2 | `CSMergeSH` | `(1, 1, 1)`, `64` threads | 归约 512 个 tile 结果 → 标准化 + cosine convolution → 9 个最终 SH 系数写入 `_AtmosphereSH` |

旧实现为 `[numthreads(1,1,1)]` 单线程遍历 512×256 = 131,072 个 texel，在 RTX 5070 上耗时 10+ ms。并行化后降至 ~0.1 ms（约 100× 提速）。

每个 tile 内的 tree reduction 有 8 步 barrier（256 → 128 → 64 → 32 → 16 → 8 → 4 → 2 → 1），每步执行 9 个 SH 系数的 float3 加法。Merge 阶段同理，64 线程 → 单值需要 6 步 barrier。

**Unity SH 打包**（匹配 `EntityLighting.hlsl` 的 `SampleSH9`）：

```
shAr = (c3, c1, c2, c0)     shBr = (c4, c5, 3·c6, c7)
shAg = (c3, c1, c2, c0)     shBg = (c4, c5, 3·c6, c7)
shAb = (c3, c1, c2, c0)     shBb = (c4, c5, 3·c6, c7)
shC  = (c8, c8, c8, 1)
```

其中 `c_i.rgb` = SH 系数 i 的 RGB 通道。系数顺序：`[Y00, Y1-1, Y10, Y11, Y2-2, Y2-1, Y20, Y21, Y22]`。

**Assets**：`Shaders/SHConvolution.compute`，在 Feature Inspector 中赋给 `SH Convolution Compute` 字段。

### 2.5 Aerial Perspective LUT
